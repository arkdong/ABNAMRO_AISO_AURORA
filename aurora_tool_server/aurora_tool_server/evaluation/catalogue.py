...
Reads ``assets/evaluation/kpi_catalogue.json`` (built from ABN AMRO's
``Content KPI inventory_AISO.xlsx`` workbook by the catalogue build script)
and exposes typed accessors. Loaded once via ``lru_cache``; the file is small
(~130 entries) so reload cost is negligible if a test calls
...
from ..schemas import Channel, Origin

CATALOGUE_PATH = EVALUATION_DIR / "kpi_catalogue.json"
...
class Catalogue:
    ...
    def applicable(
        self, *, origin: Origin, channel: Channel, include_conditional: bool = False
    ) -> tuple[KPI, ...]:
        ...
        Some workbook cells read ``"Only applicable for GenAI source"`` —
        e.g. tracability and approved-source for origin ``instant``. With
        ``include_conditional=True`` those KPIs are included too, for
        consumers whose checks self-condition on the content (tier 1 source
        checks only fire when citations exist). The dCLP signoff steps keep
        the strict reading (the signoff belongs to the pre-approved source,
        not the instant output).
        """

        def state(value: Optional[str]) -> str:
            if value is None:
                return "yes"  # missing → permissive (the workbook leaves channel cells blank for "all")
            v = value.strip().lower()
            if v.startswith("only applicable"):
                return "conditional"
            if v.startswith("applicable") or v == "need":
                return "yes"
            return "no"

        allowed = {"yes", "conditional"} if include_conditional else {"yes"}
        return tuple(
            k
            for k in self.kpis
            if state(k.relevance.get(origin)) in allowed
            and state(k.relevance.get(channel)) in allowed
        )
