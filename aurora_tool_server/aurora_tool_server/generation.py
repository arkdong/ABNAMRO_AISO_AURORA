...
def _source_title(snippet: Snippet) -> str:
    return snippet.article_title or snippet.title or snippet.source_doc
...
def _source_reference(snippet: Snippet) -> str:
    return _markdown_link(_source_title(snippet), snippet.source_url)
...
def _stub_body(
    ...
    if intent.language == "nl":
        ...
        for index, snippet in enumerate(snippets[:5], start=1):
            source_lines.append(f"- [{index}] {_source_reference(snippet)}")
...
def _citations(snippets: list[Snippet]) -> list[Citation]:
    return [_citation(index, snippet) for index, snippet in enumerate(snippets[:5], start=1)]


def _citation(index: int, snippet: Snippet) -> Citation:
    return Citation(
        index=index,
        source_doc=snippet.source_doc,
        node_id=snippet.node_id,
        title=snippet.title,
        article_title=snippet.article_title or snippet.title,
...
def _citations_from_indices(
    snippets: list[Snippet],
    citation_indices: list[object],
) -> list[Citation]:
    return [
        _citation(index, snippets[index - 1])
        for index in citation_indices
        if isinstance(index, int) and 1 <= index <= len(snippets)
    ]
    ...
    *,
    ...
    if api_key and model:
        try:
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "reasoning, and citation_indices. Cite source indices in "
                            "the body and rely on the provided article-title source "
                            "links for final citation metadata.\n\n"
            ...
            citations = _citations_from_indices(snippets, citation_indices)
            if snippets and not citations:
