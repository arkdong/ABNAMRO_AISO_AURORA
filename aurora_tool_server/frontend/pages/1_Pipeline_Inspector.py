...
def _run_id(message: dict[str, Any]) -> str | None:
    return message.get("run_id") or st.session_state.get("pipeline_run_id")
...
if prompt:
    ...
    st.session_state.messages.append(
        {"role": "user", "content": prompt, "run_id": run_id}
    )
    with st.chat_message("user"):
        st.markdown(prompt)
    try:
        with st.spinner("Classifying intent via AURORA API..."):
            intent = _with_client(
                lambda c: c.classify_intent(prompt, _options(), run_id=run_id)
            )
        message = {
            "role": "assistant",
            "kind": "intent",
