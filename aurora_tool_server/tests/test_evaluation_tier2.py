...
from aurora_tool_server.evaluation.indicators import (
    INDICATOR_REGISTRY,
    AmbiguityScale,
    CompletenessScale,
    DeviationScale,
...
def test_llm_path_all_pass_when_judges_return_passing_values():
    ...
    values = {
        ...
        "GroundednessScale": GroundednessScale.full,
        "CompletenessScale": CompletenessScale.full,
        "AmbiguityScale": AmbiguityScale.none_,
        "LanguageLevelScale": LanguageLevelScale.B1,
...
def test_llm_path_single_failure_isolated():
    ...
    def parse(model, messages, response_format):  # noqa: ARG001
        ...
        scale = INDICATOR_REGISTRY[scale_name]
        # Pick the first non-unknown member; if none, return the first.
        for v in scale:
            if v.value != "unknown":
